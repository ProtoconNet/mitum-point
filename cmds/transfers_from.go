...
func (cmd *TransfersFromCommand) createOperation() (base.Operation, error) { // nolint:dupl}
	...
	item := point.NewTransfersFromItem(cmd.contract,
		cmd.receiver, cmd.target, cmd.Amount.Big, cmd.Currency.CID)

	fact := point.NewTransfersFromFact(
		[]byte(cmd.Token), cmd.sender, []point.TransfersFromItem{item},
	)

	op := point.NewTransfersFrom(fact)
